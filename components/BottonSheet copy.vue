        ...
        if (event.deltaY !== 0) {
            ...
            if (type === "main" && event.type === "panup") {
                const newValue =
                    ...
                    pixelToVh(event.deltaY * SENSITIVITY);
                translateValue.value = Math.max(0, newValue); // 음수가 되지 않도록 처리
                if (event.cancelable) {
            ...
            if (
                ...
                const newValue =
                    translateValue.value +
                    pixelToVh(event.deltaY * SENSITIVITY);
                translateValue.value = Math.min(MAX_TRANSLATE_VALUE, newValue); // 최대값을 적용
            }
